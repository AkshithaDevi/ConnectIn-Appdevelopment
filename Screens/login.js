...
const Login = ({ navigation }) => {
  ...
  const handleLogin = () => {
    ...
    const url = `https://mevn.in/apps/resume/login.php?username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`;
    ...
    fetch(url)
      ...
      .then(text => {
        ...
        if (jsonResponse.status === 'success') {
          // Save username and user_id to AsyncStorage
          AsyncStorage.setItem('username', username);
          AsyncStorage.setItem('user_id', jsonResponse.user_id.toString()); // Ensure user_id is stored as a string
